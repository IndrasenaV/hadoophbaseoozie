...
public class OSAccess {
	...
	public static class Mapper extends
		...
		protected void map(ImmutableBytesWritable rowkey, Result result,
			...
			for (Entry<byte[], byte[]> entry : columnMap.entrySet()) {
				...
				Text val = new Text(userAgent.getOperatingSystem().getName());
	...
	public static class Reduce
		...
		protected void reduce(ImmutableBytesWritable rowkey,
			...
			for (Entry<String, Integer> entry : result.entrySet()) {
				put.add(AnalyticTableConstant.OS_COLUMN_FAMILY,
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = new Job(conf, "OS Access Per Date");
		job.setJarByClass(OSAccess.class);
